...
int nodeCount(node* t, int level)
{
    int c1=0, c2=0;
    if(t)
    {
        c1=nodeCount(t->left, level-1);
        c2=nodeCount(t->right, level-1);
        if(level<0)
        {
            return c1+c2+1;
        }
...
int leafCount(node* t, int level)
{
    int c1=0, c2=0;
    if(t)
    {
        c1=leafCount(t->left, level-1);
        c2=leafCount(t->right, level-1);
        if(level<0 && !t->left && !t->right)
        {
            return c1+c2+1;
        }
        else
...
int main()
{
    ...
    cout << "Write root";
    print2D(root);
    cout<< "node count: " <<nodeCount(root,2) << endl;
    cout<< "leaf count: " <<leafCount(root,3) << endl;
}
